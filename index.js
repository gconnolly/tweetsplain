                      ...
                      request(
                        {
                          ...
                          method: 'POST',
                          json: {
                            'params': 'query=%22' + encodeURIComponent(tweet.text) + '%22&dateRange=pastMonth'
                          }
                        },
                        (error, response, body) => {
                          if (error) {
                            ...
                            if (body && body.hits && body.hits[0]) {
                              resolve('@' + req.body.username + ' https://hn.algolia.com/?query=' + encodeURIComponent(tweet.text) + '&type=all&numericFilters=%5B%22created_at_i%3E' + new Date().setMonth(new Date().getMonth() - 1) + '%22%5D ' + body.hits[0].story_url)
                            } else {
